...
class ScaledsoftCrossEntropy(nn.Module):
    def __init__(self, ignore_index= -1, gamma=0):
    ...
    def forward(self, inputs, target):
        ...
        pred_P = F.softmax(inputs, dim=1)
        
        if self.gamma:
            pred_P = (1-2*self.gamma)*pred_P+self.gamma
            target = (1-2*self.gamma)*target+self.gamma
        log_likelihood = - torch.log(pred_P)
